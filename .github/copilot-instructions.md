...
- The whole UI is jQuery + Select2 + SweetAlert2 + Tippy + Dropzone + ApexCharts, all loaded from CDNs in each page's `<head>`. Do not add npm/vite/yarn. Scroll animations use ScrollReveal (reversible, `reset: true`) and the page background is a three.js WebGL canvas — both also CDN.
...
- Multi-user live updates: each dashboard client polls `refresh_check.php` every ~10s, sending its current filters + a hash (`sig`) of the table it's showing. The server re-runs the same filtered query, re-hashes, and only answers `changed:true` if the hash differs; the client then calls `window.quietRefresh(..., { resetForm:false })`. Guards in `script.js` skip the refresh while the "Add order" form has any filled field, while the order slider/modal is open, and ~10s after the local user's own submit. `refresh_check.php` must keep `$limit = 18` and the WHERE/ORDER logic in sync with `dashboard.php`. It returns two signatures: `pageSig` (table+pagination+pinned) and `statsSig` (global stat-card aggregates). If only `statsSig` differs (e.g. an added order excluded by the user's filters), the client updates just `.stats-banner` via `window.animateStatCards` instead of a full refresh.
- Stat cards = filter shortcuts: each `.stat-card` carries `data-status-filter` and is clickable — it toggles a "smart" `status_filter` bucket and quiet-refreshes the table. `dashboard.php` maps `overdue`/`deliver_today`/`delivered_today` to date-aware conditions; keep `$smart_status_filters` in sync between `dashboard.php` and `refresh_check.php`.
...
- `sript.js` (note the typo in the filename) is a small helper file used by the add-order form; most other JS is inlined in each page.
