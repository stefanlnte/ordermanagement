 ...
 * Select2, Tippy/Popper, ScrollReveal, three.js and SweetAlert2,
 * since several sections below depend on the globals those
 * libraries expose ($, Swal, tippy, ScrollReveal, THREE).
 * ------------------------------------------------------------------
 */
...
/* ============================================================
 * SECTION: ScrollReveal animations (reversible)
 * ------------------------------------------------------------
 * Every [data-gsap] element gets a scroll reveal with
 * `reset: true`, which means the reveal replays EVERY time the
 * element enters the viewport — scrolling down or scrolling back
 * up — and un-reveals when it leaves. That is the reversible
 * behaviour requested for this dashboard.
 *
 * EXCEPTION — the sticky #header family (header, search and the two
 * header buttons) uses `reset: false`: a sticky bar never leaves the
 * screen, so it gets a one-time intro animation on load only.
 *
 * One ScrollReveal instance per animation type so the
 * delays/intervals of one type never leak into another. The stat
 * cards reveal together with their banner (staggered via
 * `interval`), and the footer logo/links reveal with the footer.
 *
 * Elements already inside the viewport on load reveal on
 * ScrollReveal's first pass (short-page friendly).
 *
 * Respects prefers-reduced-motion: skipped entirely below.
 * ============================================================ */
document.addEventListener('DOMContentLoaded', function () {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  if (typeof ScrollReveal === 'undefined') return;

  // Shared behaviour: reversible + tidy up inline styles after each
  // reveal so the elements end in their natural CSS state.
  var BASE = { reset: true, cleanup: true, viewFactor: 0.15, mobile: true };

  var SR_ANIMS = {
    // NOTE: the sticky #header container itself never animates — a
    // sticky bar never leaves the screen, so replaying its reveal
    // looks broken. Its CONTENTS (search + the two header buttons)
    // get one-time intro animations via their `reset: false` defs.
    search: {
...
/* ============================================================
 * SECTION: three.js WebGL background (print shop: ink in water)
 * ------------------------------------------------------------
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Safety net: ensure the dark scene backdrop class is present (it is
  // normally already set server-side on <html> in dashboard.php).
  ...
  for (var p = 0; p < PARTICLE_COUNT; p++) {
    ...
    positions[p * 3 + 1] = (Math.random() - 0.5) * 44;
  ...
  function readScroll() {
    ...
    scrollProgress = Math.min(window.pageYOffset / max, 1);
  }
  readScroll();
  ...
  function render() {
    ...
    // Velocity swirl relaxes back to rest when scrolling stops.
    velocityBoost *= 0.94;

    // Camera: vertical travel + gentle arc across the scroll range.
    ...
    for (var j = 0; j < PARTICLE_COUNT; j++) {
      var y = posAttr.array[j * 3 + 1] + 0.008 + velocityBoost * 0.01;
      if (y > 22) y = -22;
      posAttr.array[j * 3 + 1] = y;
      posAttr.array[j * 3] += Math.sin(t * 0.4 + j) * 0.0016;
    }
    posAttr.needsUpdate = true;
    dust.position.y = scrollProgress * 5;
    dust.rotation.y = t * 0.02 + scrollProgress * 0.6;

    // Keep-out: dots inside the orders table's on-screen area are
    // pushed to the zone edge, so no dot ever floats under the text.
    // Only dots near the table's depth plane (|z| < 6) are affected —
    // far background dust stays visible around the table.
    if (tableEl) {
      ...
      if (rect.bottom > 0 && rect.top < window.innerHeight && rect.width > 0) {
        ...
        for (var k = 0; k < PARTICLE_COUNT; k++) {
          if (Math.abs(posAttr.array[k * 3 + 2]) > 6) continue;
          var fx = (posAttr.array[k * 3] - zx) / zhw;
          var fy = (posAttr.array[k * 3 + 1] + dust.position.y - zy) / zhh;
          if (fx * fx + fy * fy < 1) {
            posAttr.array[k * 3] = zx + (fx >= 0 ? 1 : -1) * zhw * 1.05;
          }
        }
      }
    }

    // Registration grid feeds like paper through a press — clearly
    // scroll-linked (reverses when scrolling up) with a velocity kick.
    grid.position.z = (scrollProgress * 14 + velocityBoost * 0.6) % 2;
    grid.position.x = Math.sin(scrollProgress * Math.PI) * 2;
