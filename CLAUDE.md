...
- Multi-user live updates: each dashboard client polls `refresh_check.php` every ~10s, sending its current filters (`status_filter`/`assigned_filter`/`category_filter`/`client_filter`/`sort_order`/`page`) + a hash (`sig`) of the table it's showing. The server re-runs the same filtered query, re-hashes, and only answers `changed:true` if the hash differs; the client then calls `window.quietRefresh(..., { resetForm:false })`. This makes a change show up for OTHER users but never when the user's active filters wouldn't show it. Guards in `script.js` skip the refresh while the "Add order" form has any filled field, while the order slider/modal is open, and for ~10s after the local user's own submit. `refresh_check.php` must keep `$limit = 18` and the WHERE/ORDER logic exactly in sync with `dashboard.php`. `refresh_check.php` returns **two** signatures: `pageSig` (table + pagination + pinned) and `statsSig` (the global stat-card aggregates over ALL orders). If only `statsSig` differs — e.g. an added order that the user's filters exclude — the client updates just the `.stats-banner` numbers via `window.animateStatCards` instead of a full refresh.
- Stat cards = filter shortcuts: each `.stat-card` is clickable and carries `data-status-filter`. Clicking toggles a "smart" `status_filter` bucket (`overdue`, `assigned`, `completed`, `deliver_today`, `delivered_today`) and quiet-refreshes the table (`dashboard.php` maps `overdue`→`status='assigned' AND due_date<CURDATE()`, `deliver_today`→`... =CURDATE()`, `delivered_today`→`status='delivered' AND DATE(delivery_date)=CURDATE()`). The `$smart_status_filters` map in `dashboard.php` must stay in sync with the identical map in `refresh_check.php` and the stat-card `data-status-filter` attributes.
